...
public class Save : MonoBehaviour
{
        ...
        if (transform.childCount > 0)
        {
            ...
            if (img != null && img.sprite != null)
            {
                ...
                if (nomImage != dernierNomImage || different)
                {
                    ...
                    PlayerPrefs.SetFloat(name + "VieEnfant", vie.fillAmount);
    ...
    public void refresh()
    {
        ...
        if (PlayerPrefs.HasKey(name + "NomImageEnfant"))
        {
            ...
            // NOUVEAU : On applique la vie sauvegardée à l'interface visuelle (fillAmount)
            Image vierefresh = gifInstance.transform.Find("vie").Find("compteur").GetComponent<Image>();
            if (PlayerPrefs.HasKey(name + "VieEnfant"))
            {
                vierefresh.fillAmount = PlayerPrefs.GetFloat(name + "VieEnfant");
            }
            else
            {
                vierefresh.fillAmount = 1f; // Valeur par défaut
            }
                        
            // Reste de ton code intact
            if (PlayerPrefs.GetFloat(name + "VieEnfant") > 0)
            {
                ApplyImage(dernierNomImage, gifInstance);
            }
            else
            {
                ApplyImage(dernierNomImage + "broken", gifInstance);             
            }
    ...
    private void ApplyImage(string Name, GameObject imagemineur)
    {
        if (imagemineur == null)
        {
        ...
        else
        {
            ...
            if (loaded == null)
            {
                ...
                imagemineur.GetComponent<InformationMineur>().stockinfo_special_bool = true;
            }
            if (loaded == null)
            {
