...
public class CheckOtherCanvas : MonoBehaviour
{
    ...
    void OnButtonClick()
    {
        ...
        DecrementSpriteCount(selectedBaseName, PlayerPrefs.GetInt("selectspeed"), PlayerPrefs.GetInt("selectheat"), PlayerPrefs.GetFloat("selectvie"), PlayerPrefs.GetInt("selectupspecial"));
    ...
    private void AddPrefabToScene(string baseName)
    {
        ...
        if (sprite == null)
        {
            sprite = Resources.Load<Sprite>("specialminer/" + baseName);
            gifInstance.GetComponent<InformationMineur>().stockinfo_special_bool = true;
        }
        ...
        PlayerPrefs.SetInt(parentTarget.name + "UpSpecial", PlayerPrefs.GetInt("selectupspecial"));
        PlayerPrefs.Save();
        if (sprite != null)
        {
    ...
    public void DecrementSpriteCount(string targetName, int targetSpeed, int targetHeat, float targetvie, int targetUpSpec)
    {
        if (!File.Exists(filePath2))
        {
        ...
        for (int i = 0; i < data.spriteCounts.Length; i++)
        {
            ...
            if (sprite.baseName == targetName && 
                sprite.upspeed == targetSpeed && 
                sprite.upheat == targetHeat && 
                Mathf.Abs(sprite.vie - targetvie) < 0.01f &&
                sprite.upspecial == targetUpSpec)
            {
        ...
        if (!found)
        {
            Debug.LogWarning($"Objet non trouvé : {targetName} avec speed={targetSpeed}, heat={targetHeat} et vie={targetvie} et upsecial={targetUpSpec}");
            return;
        }
