...
public class recapscript : MonoBehaviour
{
    ...
    void Awake()
    {
        // On sauvegarde les positions initiales ici, UNE SEULE FOIS.
    ...
    private void CheckIdleTime()
    {
        ...
        if (PlayerPrefs.GetString("Didactitiel") == "true")
        {
            ...
            //timeAway = new TimeSpan(10, 0, 0, 0);
            ...
            {
                PlayerPrefs.SetInt("mutlirecap", 32);
                PlayerPrefs.Save();
    ...
    public void BoutonPasserRecap()
    {
        if (timeAway.TotalSeconds <= 0) return;

        // On calcule combien de "boucles" il restait à faire
        int mutli = PlayerPrefs.GetInt("mutlirecap", 1);
        double minutesPerTick = 3.0 * mutli;
        int ticksRestants = (int)(timeAway.TotalMinutes / minutesPerTick);

        // On récupère tous les mineurs de la scène
        Argent[] tousLesArgents = FindObjectsByType<Argent>(FindObjectsSortMode.None);
        vie_mineur[] toutesLesVies = FindObjectsByType<vie_mineur>(FindObjectsSortMode.None);

        // On simule instantanément le temps restant
        for (int i = 0; i < ticksRestants; i++)
        {
            // 1. On diminue la vie en premier
            foreach (var vie in toutesLesVies)
            {
                vie.SimulerTickOffline(mutli);
            }
            // 2. On génère l'argent (si la vie n'est pas à 0)
            foreach (var argent in tousLesArgents)
            {
                argent.SimulerTickOffline(0.1f, mutli);
            }
        }

        // On met le temps restant à zéro. 
        // Ta coroutine decreasetime() va s'en rendre compte au prochain passage et fermera proprement l'UI !
        timeAway = TimeSpan.Zero;
    }
