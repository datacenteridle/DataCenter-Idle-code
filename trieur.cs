...
public class trieur : MonoBehaviour
{
    ...
    private double GetHeatFromTexture(string texture2DName)
    {
        ...
        if (pathNormal != null)
        {
            ServeursList dataNormal = JsonUtility.FromJson<ServeursList>(pathNormal.text);
            if (dataNormal != null && dataNormal.serveurs != null)
            {
                foreach (var serveur in dataNormal.serveurs)
                {
                    if (serveur.texture2D == texture2DName) return serveur.heat;
                }
            }
        }
        ...
        if (pathSpecial != null)
        {
            ServeursList dataSpecial = JsonUtility.FromJson<ServeursList>(pathSpecial.text);
        ...
        return 0; // Si vraiment introuvable
    }
    ...
    private double GetSpeedFromTexture(string texture2DName)
    {
        ...
        if (pathNormal != null)
        {
            ...
            if (dataNormal != null && dataNormal.serveurs != null)
            {
                foreach (var serveur in dataNormal.serveurs)
                {
                    if (serveur.texture2D == texture2DName) return serveur.vitesse;
                }
            }
        }
