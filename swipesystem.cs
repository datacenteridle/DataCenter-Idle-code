...
public class SwipeSystem : MonoBehaviour
{
    ...
    public void OnSwipeBegin()
    {
        ...
        isDragging = true;
        factorupdown = 1f;
        initialPos = new Vector2(-width * currentPage, canva.anchoredPosition.y);
    ...
    public void OnSwipeCancelled()
    {
        ...
        moveCoroutine = StartCoroutine(SmoothMove(canva.anchoredPosition, initialPos));
    }
    ...
    public void OnSwipe(string swipe)
    {
        ...
        // --- CORRECTION ICI ---
        if (moveCoroutine != null) StopCoroutine(moveCoroutine);
        moveCoroutine = StartCoroutine(SmoothMove(canva.anchoredPosition, targetPos));
    }
    ...
    void Update()
    {
        ...
        if (PlayerPrefs.GetString("DidactitielSwipe") == "false" || PlayerPrefs.GetString("drag") == "true" || Canvas_Arrive.alpha != 0 || Canvas_info_mineur.alpha == 1 || isNavigating)
        {
            // Si on était en train de draguer et qu'on est soudainement bloqué, on annule !
            if (isDragging) 
            {
                isDragging = false;
                if (moveCoroutine != null) StopCoroutine(moveCoroutine);
                // On retourne à la page actuelle proprement
                moveCoroutine = StartCoroutine(SmoothMove(canva.anchoredPosition, new Vector2(-width * currentPage, canva.anchoredPosition.y)));
            }
            return; // On arrête tout l'Update ici
        }
