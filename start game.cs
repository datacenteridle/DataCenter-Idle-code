...
public class startgame : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        audioSource.volume = PlayerPrefs.GetFloat("music", 0.5f) * 0.4f;
        audioSource.Play();
    ...
    private static void LoadServeurData()
    {
        ...
        // 1. Charger les machines normales
        TextAsset pathNormal = Resources.Load<TextAsset>("Mineur_data");
        if (pathNormal != null)
        {
            cachedServeurData = JsonUtility.FromJson<ServeursList>(pathNormal.text);
            if (cachedServeurData != null && cachedServeurData.serveurs != null)
            {
        ...
        // 2. Charger les machines spéciales et les ajouter au même dictionnaire
        TextAsset pathSpecial = Resources.Load<TextAsset>("Mineur_data_special");
        if (pathSpecial != null)
        {
            ServeursList cachedSpecialData = JsonUtility.FromJson<ServeursList>(pathSpecial.text);
            if (cachedSpecialData != null && cachedSpecialData.serveurs != null)
            {
                foreach (var serveur in cachedSpecialData.serveurs)
                {
                    // On les ajoute au même dictionnaire !
                    serveurByTexture[serveur.texture2D] = serveur; 
                }
            }
        }
        ...
        {
            Debug.LogWarning("Mineur_data_special introuvable dans Resources. Les machines spéciales seront ignorées.");
        }
    }
    ...
    public void RefreshActiveMiners()
    {
        ...
        foreach (Transform child in canvasTransform)
        {
            ...
            {
                if(GetTypeFromTexture(imageName) == "D")
                {
                ...
                else if(GetTypeFromTexture(imageName) == "C")
                {
                    activeMiners[child.name] = new MinerInfo
                    {
                        spriteName = imageName,
                        ...
                        baseHeat = serveur.heat - PlayerPrefs.GetInt(child.name +"UpSpecial", 0),
                        cell = serveur.cell
                    };
                }
                else
    ...
    void Update()
    {
        if (Pointer.current != null)
        {
            ...
            // On met à jour la position uniquement si elle n'est pas (0,0)
            // Cela évite le bug du "clic trop rapide" qui renvoie au point d'origine (en bas à gauche)
        ...
        {
            nextStatsUpdate = Time.time + 0.1f;
    ...
    private void CalculateStats()
    {
        ...
        foreach (var miner in activeMiners.Values)
        {
            ...
            heat = heat - PlayerPrefs.GetInt(miner.spriteName + "Star", 0);

            // Vitesse
            double speedValue = (miner.baseSpeed * (1 + GetValueSpeed(miner.upgradeSpeed))) + (miner.baseSpeed * (PlayerPrefs.GetInt(miner.spriteName + "Star", 0) * 0.1));
    ...
    public void Click()
    {
        ...
        double montant = double.Parse(PlayerPrefs.GetString("Bestspeed", "0"), CultureInfo.InvariantCulture) * 0.01;
        LancerAnimationPiece(lastPointerPosition); // ✅ position capturée juste avant
        user.modifargent(montant);
        user.modifargentquest(montant);
    }
    ...
    private void LancerAnimationPiece(Vector2 screenPosition)
    {
        ...
        pieceInstance.GetComponent<PieceVolante>().speed = 100f;
        pieceInstance.GetComponent<PieceVolante>().fadeDuration = 0.7f;
        pieceRect.sizeDelta = new Vector2(1, 1);
        ...
        pieceRect.anchoredPosition = localPoint + new Vector2(offsetX, offsetY);
        pieceInstance.GetComponent<PieceVolante>().targetPosition = argent.transform.position;
        AudioSource.PlayClipAtPoint(coinsound, Vector3.zero, cachedSonVolume * 0.3f);
    }
